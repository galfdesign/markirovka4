...
function App() {
  ...
  const { state, dispatch, state: { objectName } } = useContext(AppStateContext);
  const [showHistory, setShowHistory]              = useState(false);
  const makeFileName = (base: string) => {
    const prefix = objectName?.trim() || 'Новый объект';
    return `${prefix}_${base}`;
  };

  // Деструктурируем ключевые поля из глобального state
  const {
          floorCards,
          floorCollectorName,
          floorPhoto,
  ...
  // Обёртки-сеттеры, которые диспатчат в наш контекст
  const setFloorCards         = (cards: CardData[]) =>
    dispatch({ type: 'EDIT', payload: { floorCards: cards } });
  const setFloorCollectorName = (name: string) =>
    dispatch({ type: 'EDIT', payload: { floorCollectorName: name } });
  const setFloorPhoto         = (photo: string | null) =>
    dispatch({ type: 'EDIT', payload: { floorPhoto: photo } });

  // … аналогично для воды и радиаторов …
  const setWaterCards         = (cards: CardData[]) =>
    dispatch({ type: 'EDIT', payload: { waterCards: cards } });
  const setWaterCollectorName = (name: string) =>
    dispatch({ type: 'EDIT', payload: { waterCollectorName: name } });
  const setWaterPhoto         = (photo: string | null) =>
    dispatch({ type: 'EDIT', payload: { waterPhoto: photo } });

  const setRadiatorCards         = (cards: CardData[]) =>
    dispatch({ type: 'EDIT', payload: { radiatorCards: cards } });
  const setRadiatorCollectorName = (name: string) =>
    dispatch({ type: 'EDIT', payload: { radiatorCollectorName: name } });
  const setRadiatorPhoto         = (photo: string | null) =>
    dispatch({ type: 'EDIT', payload: { radiatorPhoto: photo } });
  // const [activeSection, setActiveSection] = useState('floor');
  // const { saveSnapshot } = useContext(AppStateContext);
  // const [showHistory, setShowHistory] = useState(false);
  ...
  const handleServerPdfTest = async () => {
    ...
    try {
      ...
      if (body.pdf) {
        ...
        const file = new File([pdfBlob], makeFileName(fileName), { type: 'application/pdf' });
          ...
          <>
            <div className="apple-card-wrapper">
              <div style={{ marginBottom: 16 }}>
                <button className="apple-create-btn--centered" style={{
                  background: '#eee',
                  color: '#222',
                  width: '100%',
                ...
                <input id="photo-input" type="file" accept="image/*" style={{ display: 'none' }}
                       onChange={handleFloorPhotoChange} ref={floorPhotoInputRef}/>
                {floorPhoto && (
                  <div style={{ marginTop: 12, display: 'flex', justifyContent: 'center' }}>
                    <div style={{ position: 'relative', display: 'inline-block' }}>
                      <img src={floorPhoto} alt="Загруженное фото" style={{
                        maxWidth: '100%',
                        maxHeight: 220,
                        borderRadius: 10,
                      ...
                      <button onClick={() => {
                        ...
                        <svg width="18" height="18" viewBox="0 0 20 20" fill="none" stroke="#ff3b30" strokeWidth="2">
              ...
              <CollectorCard collectorName={floorCollectorName} setCollectorName={setFloorCollectorName}
                             loops={floorCards}/>
              ...
              <button className="apple-create-btn--centered" style={{
                ...
                color: '#fff',
                display: 'block'
              }} onClick={handleServerPdfTest}>
                Экспорт PDF
              </button>
            </div>
            <details style={{ marginTop: 24, width: '100%', maxWidth: 1200, marginLeft: 'auto', marginRight: 'auto' }}>
              <summary style={{ fontSize: 18, fontWeight: 600, cursor: 'pointer', padding: '8px 0' }}>Результаты
            ...
            <div className="apple-card-wrapper">
              <div style={{ marginBottom: 16 }}>
                <button className="apple-create-btn--centered" style={{
                  ...
                  border: '2px solid #0071e3',
                  boxShadow: '0 0 0 1px #0071e3'
                }} onClick={() => document.getElementById('photo-input-water')?.click()}>
                  Добавить фото
                </button>
                <input id="photo-input-water" type="file" accept="image/*" style={{ display: 'none' }}
                       onChange={handleWaterPhotoChange} ref={waterPhotoInputRef}/>
                {waterPhoto && (
                  <div style={{ marginTop: 12, display: 'flex', justifyContent: 'center' }}>
                    <div style={{ position: 'relative', display: 'inline-block' }}>
                      <img src={waterPhoto} alt="Загруженное фото" style={{
                        maxWidth: '100%',
                        maxHeight: 220,
                        borderRadius: 10,
                        boxShadow: '0 2px 8px rgba(0,0,0,0.08)'
                      ...
                      <button onClick={() => {
                        setWaterPhoto(process.env.PUBLIC_URL + '/voda.png');
                        if (waterPhotoInputRef.current) waterPhotoInputRef.current.value = '';
                      }} style={{
                        position: 'absolute',
                        top: 6,
              ...
              <WaterCollectorCard collectorName={waterCollectorName} setCollectorName={setWaterCollectorName}
                                  loops={waterCards}/>
              ...
              <button className="apple-create-btn--centered" style={{
                ...
                color: '#fff',
                display: 'block'
              }} onClick={handleServerPdfTestWater}>
                Экспорт PDF
              </button>
            </div>
            <details style={{ marginTop: 24, width: '100%', maxWidth: 1200, marginLeft: 'auto', marginRight: 'auto' }}>
            ...
            <div className="apple-card-wrapper">
              <div style={{ marginBottom: 16 }}>
                <button className="apple-create-btn--centered" style={{
                  ...
                  border: '2px solid #0071e3',
                  boxShadow: '0 0 0 1px #0071e3'
                }} onClick={() => document.getElementById('photo-input-radiator')?.click()}>
                  Добавить фото
                </button>
                <input id="photo-input-radiator" type="file" accept="image/*" style={{ display: 'none' }}
                       onChange={handleRadiatorPhotoChange} ref={radiatorPhotoInputRef}/>
                {radiatorPhoto && (
                  <div style={{ marginTop: 12, display: 'flex', justifyContent: 'center' }}>
                    <div style={{ position: 'relative', display: 'inline-block' }}>
                      <img src={radiatorPhoto} alt="Загруженное фото" style={{
                        maxWidth: '100%',
                        maxHeight: 220,
                        borderRadius: 10,
                        boxShadow: '0 2px 8px rgba(0,0,0,0.08)'
                      ...
                      <button onClick={() => {
                        setRadiatorPhoto(process.env.PUBLIC_URL + '/rad.png');
                        if (radiatorPhotoInputRef.current) radiatorPhotoInputRef.current.value = '';
                      }} style={{
                        position: 'absolute',
                        top: 6,
              ...
              <RadiatorCollectorCard collectorName={radiatorCollectorName} setCollectorName={setRadiatorCollectorName}
                                     loops={radiatorCards}/>
              {radiatorCards.map((card, index) => (
                <div key={index}>
                  <RadiatorAppleCard
                    ...
                    onEdit={() => {
                      setRadiatorModalData({ ...radiatorCards[index], index });
              ...
              <button className="apple-create-btn--centered" style={{
                ...
                color: '#fff',
                display: 'block'
              }} onClick={handleServerPdfTestRadiator}>
                Экспорт PDF
              </button>
            </div>
            <details style={{ marginTop: 24, width: '100%', maxWidth: 1200, marginLeft: 'auto', marginRight: 'auto' }}>
        ...
        <div className="modal-backdrop">
          <div className="modal-window modal-small">
            <div className="apple-card-field" style={{ marginBottom: 8 }}>
              <label style={{ textAlign: 'center', width: '100%', fontSize: 16, fontWeight: 600, marginBottom: 6 }}>Петля
                №{floorModalData.index + 1}</label>
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
              <button className="apple-create-btn--centered"
                      style={{ width: '100%', background: '#0071e3', fontSize: 15, padding: '10px 0', margin: 0 }}
                      onClick={handleSave}>
                Сохранить
              </button>
              <button className="apple-create-btn--centered" style={{
                ...
                padding: '10px 0',
                margin: 0
              }} onClick={() => setFloorShowModal(false)}>
                Отмена
              </button>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <svg width="48" height="48" viewBox="0 0 50 50" style={{ marginBottom: 16 }}>
              <circle cx="25" cy="25" r="20" stroke="#0071e3" strokeWidth="5" fill="none" strokeDasharray="31.4 31.4"
                      strokeLinecap="round">
                <animateTransform attributeName="transform" type="rotate" repeatCount="indefinite" dur="1s"
                                  values="0 25 25;360 25 25" keyTimes="0;1"/>
              </circle>
            </svg>
            Формируется PDF...
          </div>
